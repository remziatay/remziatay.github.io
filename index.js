...
const windowLoaded = () => {
  ...
  popUpElem.hide(1);
  popUpElem.on('click', e => e.stopPropagation());
  ...
  const closePopUp = ()=>{
    ...
    if ($(window).width() >= 768) {
      ...
      $('.intro').css('filter', 'none');
      mainElem.css('filter', 'none');
      $('body').css('overflow', 'auto');
    } else {
      // if been viewed on mobile just take away all element except popUp.
      mainElem.css('display', 'block')
    }
  ...
  mainElem.on('click', e => {
    ...
    if ($(e.target).closest('.project-item').length > 0) {
      ...
      if (popUpState === false) {
        ...
        if ($(window).width() >= 768) {
          // blur the shiit outta the main element and make it unscrollable. ONLY ON TAB AND DESKTOPS
          $('.intro').css('filter', 'blur(7px)');
          mainElem.css('filter', 'blur(7px)');
        } else {
          // if been viewed on mobile just take away all element except popUp.
